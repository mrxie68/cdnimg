#!/usr/bin/bash    
dnspod_ddnsipv6_id="236848" #【API_id】将引号内容修改为获取的API的ID
dnspod_ddnsipv6_token="231e03a9422ffee3412954d8583b555c" #【API_token】将引号内容修改为获取的API的token
dnspod_ddnsipv6_ttl="600" # 【ttl时间】解析记录在 DNS 服务器缓存的生存时间，默认600(s/秒)
dnspod_ddnsipv6_domain='0412.cyou' #【已注册域名】引号里改成自己注册的域名
dnspod_ddnsipv6_subdomain='op' #【二级域名】将引号内容修改为自己想要的名字，需要符合域名规范，附常用的规范
